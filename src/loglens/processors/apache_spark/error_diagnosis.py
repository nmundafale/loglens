...
async def process_error_diagnosis(
    log_text: str, 
    intent: str,
    ...
    """Executes the Map-Reduce pipeline for Spark error diagnosis."""
    ...
    map_prompt = APP_CONFIG["prompts"]["diagnosis_map"]
    reduce_prompt = APP_CONFIG["prompts"]["diagnosis_reduce"].format(intent=intent)
    
    concurrency_limit = APP_CONFIG["ai_tiers"][map_tier].get("max_concurrent", 1)
    semaphore = asyncio.Semaphore(concurrency_limit)

    await log_status(f"--- Map Phase (Tier {map_tier} | Concurrency: {concurrency_limit}) ---")
    log_chunks = chunk_log_by_lines(log_text, chunk_size)
    
    async def bounded_call(chunk_data, chunk_index):
        async with semaphore:
            await log_status(f"Processing chunk {chunk_index + 1}/{len(log_chunks)}...")
    ...
    if not distilled_logs:
        return "No relevant Spark telemetry or errors were found in the logs."
    ...
    if github_repo and etl_file_path:
        ...
        try:
            ...
            # PyGithub is synchronous, warp it
    ...
    # 4. Reduce
    await log_status(f"--- Reduce Phase (Tier {reduce_tier}) ---")
    ...
    try:
        ...
        if github_service and github_repo and etl_file_path and fixed_code:
            ...
            # PyGithub is synchronous and blocks the asyncio loop, so we must wrap the PR execution in a thread
            pr_url = await asyncio.to_thread(
                github_service.create_pull_request,
                repo_name=github_repo,
                file_path=etl_file_path,
                new_code=fixed_code,
                pr_title=f"Loglens Auto-Fix: {etl_file_path}",
                pr_body=f"Automated PR created by Loglens AI. \n\n### Pipeline Metrics\n- **{metrics_str}**\n- Map-Reduce Execution Time: {total_duration:.2f}s\n\n### Diagnosis\n{final_analysis}"
            )
            if pr_url:
